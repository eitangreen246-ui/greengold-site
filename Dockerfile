...
# Collect static files (build-time only needs a throwaway key; real secrets are
# injected at runtime by the host).
RUN SECRET_KEY=build-time-only ALLOWED_HOSTS=localhost \
...
CMD set -xe; python manage.py migrate --noinput; gunicorn greengold.wsgi:application --bind 0.0.0.0:$PORT --workers 3
